...
9. **Booking-detail two-column rebuild (in progress, Jun 2 2026).** Desktop page (`BookingDetailPage.jsx` left box + `DetailPanel` page mode in `ScheduleDashboard.js`) being rebuilt to match the approved mockup, one modular step at a time. Rollback tag `stable-before-2col` = `ddd4ece4`. Done so far: Step 1 (cadence/insight + booking-note moved to left box, `51bfdeed`), visual cleanup (cadence up top, unified soft Notes box, Schedule pill, `077f691a`), Step 2 (right-column ACTIONS card: Checkout/Book-next/Reschedule/No-show/Cancel, `1e53a8fd`). Remaining: **Step 3** Setup block (Send intake Email/SMS + Agreement) to left box; **Step 4** payment block (the green "Paid $X" receipt card + Refund, currently still rendered in the panel on the right) moves to the LEFT box bottom per mockup, and the panel's paid card is then removed in page mode; **Step 5** Session card + both edit pencils to the left, remove the right date card. Each step: extract a modular block, gate panel duplicates on `(mode === 'slide' || isMobileW)` so each renders once per layout, build-verify, push only when green.
