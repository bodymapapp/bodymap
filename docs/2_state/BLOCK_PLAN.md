...
**DATA CLEANUP (Jun 3 2026, low priority, no rush).**
- Back2Life / Sandy Carder: her standing 12:15 PM appointment is saved twice on 20 future dates (Jul 22 2026 to Apr 28 2027). Same client, same slot, so it is a harmless duplicate row, not a real double-book. No data loss either way. Jacquie can delete the extra copy per date, or HK can ask her later. The new double-booking guard prevents fresh duplicates going forward.
- Grounded Grace / Candice Peek: a real two-client clash on Jun 26 2026. Caleb Allen 11:30 to 1:15 (a 75-minute service that ended up 105 minutes) overlaps Lindsay Barnes 1:00 to 2:00 by 15 minutes. Candice needs to shorten Caleb or move Lindsay. HK is hesitant to reach out (prior bad experience) but it needs a gentle heads-up. Runway is 3+ weeks.
...
- Availability date overrides: table + booking respects them, Schedule hours bar + day sheet, Settings date-specific editor with multi-date add (Method 1) and copy-a-week-forward (Method 2) → `c83fed2d` `32f097c9` `dc2fd4b8` + Method 2. Founder page Test Plan card (durable) → `e4fec168`. Marketing surfaced: Home Find & Book gets a DateOverrideDemo SVG animation + sub-feature line, Features 1.4 "Availability & hours" card expanded to cover date-specific hours. Split-shift editing in the override sheet still deferred.
...
- 2026-06-11 — **SMS status corrected (per HK).** A2P 10DLC / carrier registration is cleared as of roughly early June, so SMS is NOT blocked at the carrier level anymore. The old "texting blocked pending carrier registration" line in GLOSSARY was stale and was causing wrong customer answers; corrected. Open issue: HK cannot get live SMS sends working yet (delivery not firing). That is a real engineering item, separate from the waitlist build. Until sends are confirmed, any texting feature must fall back to email. (The "SMS stays off for this self-action" pay-link note at the top of this plan is an intentional design choice, not a block, and is unchanged.)
...
- 2026-06-12 — **[support] Ashley / Puro Glow, two inbound emails (mobile booking + day-of payment). Four NEW asks, none logged before, none broken (all are not-yet-built feature requests, not incidents).**
  1. **Client address at booking (mobile therapist).** She wants the client to enter the address the massage is brought to, shown to her on the request *before* she approves or declines. NOT built today: the booking page collects no client service address, and "location" in the schedule is the therapist's own fixed studio locations (therapist_locations), not a client-entered address. HK already replied in-thread "should be easy, will check today." Feasibility note for engineering: the clients table already has 6 address columns (line1/line2/city/state/zip/country, migration 2026-05-21), so the gap is capturing the address at booking time and surfacing it on the approve/decline view, not new schema.
  2. **Auto-fill the remaining balance at checkout.** She wants the day-of "make a payment" to show what the client still owes (price minus deposit already paid) instead of a free-form number she types. NOT built today: CheckoutModal defaults the amount to the full service_price_cents, not price-minus-deposit. Feasibility note: the booking row already stores deposit_required / deposit_paid / deposit_amount, so the remaining balance is computable; this is a default-value change, not new data.
  3. **Add-on button at checkout.** A button to add an add-on to the balance day-of, for when a client asks once she arrives. NOT built today: CheckoutModal has no add-on path (add-ons are set earlier on the booking/service, not at checkout).
  4. **Square Tap to Pay.** She wants a "tap to pay" that opens her Square Tap to Pay so everything updates in one place instead of her updating platforms separately. NOT built today: the platform connects to Square for saved cards and card charging, but there is no Square Tap to Pay (in-person phone-reader) integration. Only repo hit for "tap to pay" is a test SMS string, not a feature.
  Reply drafted in Joy voice for HK to review and send. No incident to log.

- 2026-06-13 — **[support] Correction to the Angela entry above: mobile appointments ARE shipped (unverified by HK), not absent.** F1 landed via PRs #19–#39: per-service "At client location" flag (services.performed_at_client_location), come-to-you address captured at booking with autocomplete + a travel-area serviceability check, address shown on the pending request before approve/decline, come-to-you bookings forced to approval-first, confirmed bookings show address + distance + Open in Maps, returning clients pre-fill. The other three Ashley asks also shipped unverified: remaining-balance autofill at checkout (defaultAmountCents = balanceDueCents), add-on at checkout (AddOnPicker in CheckoutModal, F3), and Square Tap to Pay (square-pos deep-link + square-pos-reconcile, PR #42). My earlier "not built" read for mobile was from stale main before these merges. Still genuinely NOT built: client booking waitlist, account credit/prepaid balance, per-service payment mode (deposit/card/full is global, not per service), daily limit by appointment count (only minutes cap exists). Cluster booking remains undefined (only a scheduling nudge in code).
