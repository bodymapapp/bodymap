...
- **Google OAuth verification (Google Auth Platform).** Project "BodyMap", owner/login **bodymap01@gmail.com**. Supabase project ref `rmnqfrljoknmellbnpiy` (also listed as an authorized domain). DNS for mybodymap.app is at **Cloudflare** (nameservers `cleo.ns.cloudflare.com`, `meg.ns.cloudflare.com`); the site is proxied through Cloudflare to Vercel. Scopes in use: `calendar.events` (sensitive, shown as "View and edit events on all your calendars") plus `userinfo.email`; no restricted scopes. Goal: remove the "Google hasn't verified this app" warning therapists see on calendar connect, and lift the 100-user cap.
  ...
  - Part 4, justification plus demo video: DONE 2026-06-12 (resolved after the YouTube removal). The justification (corrected two-way version, 817/1000) is saved in Data Access. HK fixed the video by obscuring the personal info that triggered the removal (the email on the Google login/consent screen, plus any on-screen client data), uploaded a clean version unlisted, and updated the YouTube link in Data Access (toast confirmed "Data access changes saved!"). The new unlisted URL is intentionally not stored here since the repo is public; HK holds it. History: the first upload was removed by YouTube under the harassment/PII policy because real personal data was visible on screen.
  - Part 5, submit (Verification Center): SUBMITTED 2026-06-12. Branding was re-verified first (the leaf logo was kept and verified in seconds), the verification questionnaire was answered No to all exemption questions (production app, external users), and the sensitive-scope (calendar.events) request was submitted with the corrected two-way justification, the fixed unlisted demo video (youtu.be/eG0dIh6euYI, PII obscured), and the no-credentials Additional info. Now in Google's review queue. Expect roughly 3 to 5 business days; Google emails questions to bodymap01@gmail.com. While pending: do NOT modify the scope or branding, and keep the YouTube video live (a takedown or change mid-review can fail it). If Google suggests narrowing to calendar.app.created or calendar.events.owned, decline and give the two-way reasoning. Status is submitted/pending Google, not yet approved, and not yet confirmed by HK.
  - Ready-to-paste scope justification: "MyBodyMap is a scheduling app for independent massage therapists. When a therapist connects their Google Calendar, MyBodyMap uses calendar.events in two ways: (1) it creates, updates, and deletes an event on the therapist's own calendar for each session booked, rescheduled, or cancelled in MyBodyMap; and (2) it reads the therapist's upcoming events and marks those times as busy on the MyBodyMap booking page so clients cannot double-book over existing commitments. Both reading existing events and writing app events are required, so a write-only or app-created-only scope is not sufficient: without read access we cannot see the therapist's other commitments and would double-book them. We use this data only for these scheduling features and do not sell it, transfer it, or use it for advertising or AI training."
  ...
  - Possible Google pushback: `calendar.events` covers "all your calendars," so Google may recommend a narrower scope such as `calendar.app.created` or `calendar.events.owned`. Those would BREAK the reverse sync, which reads the therapist's existing events to block their booking slots, so they are not viable. If Google pushes, reply explaining the two-way use (write app events plus read existing events to prevent double-booking) rather than agreeing to downscope. Do not remove an approved scope until Google instructs.
  ...
  - Privacy policy (limited-use disclosure): DONE 2026-06-12, corrected same day. The source at `src/pages/Privacy.jsx` had no Google mention, so engineering added a "5b. Google Calendar Integration" section. An engineering code audit of the edge functions then showed the integration is two-way: `google-calendar-push`/`google-calendar-forward-sync` write MyBodyMap session events to the therapist's calendar, and `google-calendar-sync` reads the therapist's upcoming events (about 60 days) and stores each event's title, start, and end in `external_calendar_events` to block booking slots. Section 5b was corrected to disclose both directions (read and store, not just write), keeping the Limited Use compliance statement and the no sale/transfer/advertising/AI-training and disconnect language. The policy "Last updated" date is June 12, 2026. Live at `https://mybodymap.app/privacy` on the next deploy.
