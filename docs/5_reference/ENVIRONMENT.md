...
- **Google OAuth verification (Google Auth Platform).** Project "BodyMap", owner/login **bodymap01@gmail.com**. Supabase project ref `rmnqfrljoknmellbnpiy` (also listed as an authorized domain). DNS for mybodymap.app is at **Cloudflare** (nameservers `cleo.ns.cloudflare.com`, `meg.ns.cloudflare.com`); the site is proxied through Cloudflare to Vercel. Scopes in use: `calendar.events` (sensitive, shown as "View and edit events on all your calendars") plus `userinfo.email`; no restricted scopes. Goal: remove the "Google hasn't verified this app" warning therapists see on calendar connect, and lift the 100-user cap.
  ...
  - Part 4, justification plus demo video: BLOCKED again as of 2026-06-12. Justification text is ready and saved in Data Access (the corrected two-way version, 817/1000). The first demo video was recorded and uploaded unlisted, but YouTube REMOVED it citing the harassment policy (sharing non-public personally identifiable information: medical records, email, phone, etc.). The likely cause is real client intake/booking data visible on screen in the recording (names, phone, email, health notes), not the massage imagery. Consequence: the YouTube link saved in Data Access is now a dead link. FIX: re-record the demo using only fake/dummy test data (a fake therapist and a fake client with made-up name, phone, email, and no real medical notes), avoid showing any real personal data, re-upload unlisted, then update the YouTube link in Data Access. Re-uploading the same footage will get removed again; the footage itself must change.
  - Part 5, submit (Verification Center): HOLD. Do NOT submit until a clean re-recorded video (dummy data only) is live and its link is updated in Data Access. If HK already submitted with the now-removed video, the reviewer will hit a dead link and the review will fail, so the link must be replaced and the request resubmitted.
