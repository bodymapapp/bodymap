...
26. **Membership card visual unification with PackageSection.** ~60-90 min. Phase 2c deferred from May 24 2026 client-profile redesign. Active membership rows render with their own visual style (plan name + payment status pill + price/credits/renewal line + inline edit form + charge button block + edit/cancel buttons). Active package rows render with icon tile + kicker + title + sub-line + status pill + 4-number breakdown + AVAILABLE big number + cancel button. The two are visually adjacent but not identical. Phase 2c would restructure the top of the membership row JSX to match: 36x36 icon tile (📅 calendar), MEMBERSHIP kicker, Georgia serif title, price/credits sub-line, status pill on the right. All existing flows (payment status, charge buttons, inline edit, ad-hoc charge modal) must stay intact. Risk: the existing membership JSX is densely packed with state-dependent rendering (`isActive`, `isEditing`, `pending`, `paymentStatus`) and tightly coupled to the charge/edit/cancel handlers. Done correctly, both card types feel like siblings on the client profile.
