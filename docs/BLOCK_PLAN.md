...
**Most recent activity:** May 25 2026 (morning, Phase 20 session cockpit redesign + cron rebuild + client notifications audit). Schedule slide-over rebuilt into a full session cockpit so therapists no longer have to leave Schedule to deliver a session. Phases shipped: 20.0 data layer (currentSession + lastSession + allSessions for patterns), 20.1 cleaner status pills + single highest-priority insight line (medical flag > new client > cadence drift > zone recurrence > warm), 20.2 DocumentJourney inline with auto-scroll/auto-expand on dot tap, 20.3 Brief panel with focus/avoid chips + pressure dot meter + preferences + client notes, 20.4 BodyMapPreview inside Brief, 20.5 Medical flags panel auto-opens when flags fire, 20.6 Last session panel with focus zones + plan or legacy notes, 20.7 Patterns panel for 3+ session clients (recurring zones, avg pressure, cadence), 20.8 inline SOAP RecordEditor saves Subjective/Objective/Assessment/Plan to sessions.therapist_notes JSON, 20.9 inline RecapEditor for warm note to client. Removed redundant Open Session Record + Open Pre-Session Brief anchor buttons (replaced by inline panels). Slide-over header clickable to client profile. 20.10 payment block left as-is (already well-designed). 20.11 actions row left as-is. 20.12 SessionDetail page repurpose queued (page still works for deep-links). Cron rebuild also completed: all 6 broken crons now auth correctly via hardcoded service_role JWT, 8 client reminders fired this morning at 4am Central. Practice Pulse logic switched from sessions to bookings. **Earlier (May 24)**: Phase 13.9-13.11 findOrCreateClient rewrite, Phase 13.12 inline ClientPicker, package add-vs-charge split, yearly view clickable cells, schedule empty-state copy fix.
