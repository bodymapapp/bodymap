...
13. **TOP PRIORITY MAY 22: Unified import flow (~5-8 hrs).** Surfaced May 21 evening from Jackie. Current two-tab UI (client vs appointment) is a Maria-persona failure: she ran the appointment import successfully but her clients have no email/phone because that data lives in a separate client-list CSV she didn't know to upload. Replace with ONE button "Bring in your data" that accepts 1 or more CSVs (drag-and-drop + file picker, both visible). Auto-detect each file by header inspection: has dates+times = appointment history; has names+contact = client roster; has services+prices = services list. Cross-reference across files: emails/phones from client roster get merged onto appointment-created client records by name match. UPSERT logic for client merge already exists at ImportClients.js line 471. Old tabs hidden behind "Advanced import options" link as fallback. Ship as the first thing on May 22, then message Jackie. Standing rule: never ask therapist for their client data via DM/email, they upload through this button (HIPAA scope). Pairs with new design principles #14 (One way in, not two) and #15 (HIPAA-aware support).
