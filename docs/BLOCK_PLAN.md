...
0b. **Real customer issues from morning of May 24 (triage before code).** Three live reports from active therapists:
...
27. **Cron audit: 6 broken edge function crons.** Surfaced May 24 2026 while debugging Candice's missing renewal rows. Root cause: anon-key bearer in `net.http_post` from `pg_cron` is silently rejected by edge functions (no logs, no errors, just no invocation). Confirmed: zero practice-pulse invocations in last 7 days. Affected crons (with their broken auth patterns): #1 send-booking-reminders (no Authorization header), #2 send-reminders-daily (anon key), #3 practice-pulse-daily (anon key), #4 send-drip-daily (anon key), #5 daily-signups-digest (no Authorization header), #6 founder-digest-daily (no Authorization header). Fix pattern: `ALTER DATABASE postgres SET "app.settings.service_role_key" = '<key>'` once, then rewrite each cron to use `Bearer ' || current_setting('app.settings.service_role_key', true)` in the Authorization header. Cron #7 google-calendar-reverse-sync already uses this pattern correctly and serves as the template. Cron #9 daily-renewal-creation fixed May 24 via service_role key but still uses inline-embedded key (vulnerable to typos like the original `<KEY>` brackets bug). After fix, manually trigger each function once via Test button to backfill any missed invocations from the broken-cron window. P0 - directly affects every therapist's notifications: reminders, practice pulse digest, drip campaigns, daily signups digest, founder digest.
28. **Therapist-initiated package charge from client profile.** ~2-3 hours of integrated work. May 24 2026 design feedback from HK: "Why is the checkout any different from checking out a Service?" Current PackageSection only supports "Already paid" path - therapist records a package the client has already paid for (cash, check, Venmo, Zelle, in-person card). There's no path to charge the client's card on file or send them a pay link from the client profile. Build: tap "Charge $X for this package" button → opens CheckoutModal with package context (Mark as paid / Card on file / Enter new card / Send pay link). On success, create the `package_purchases` row AND a linked payment record. CheckoutModal currently accepts only `appt` (session) or `subscription` (membership) - extend it to accept `packagePurchase: { name, sessions, price, expiresAt }` as a third charge context. Charge handlers (chargeCardOnFile, chargeNewCard, chargeOffline, sendPayLink) need to branch on this third context and write the right rows. Until shipped, the "Already paid" workaround stands: therapist collects payment outside the app, then records the package as paid. Yellow info banner in PackageSection explains: "To charge a client's card for a package right now, share your booking link." The booking page already supports the package-buy flow (confirm-package-purchase edge function).
