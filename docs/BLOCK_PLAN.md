...
14. **DONE May 21 2026 night (commits `de8e937d` through `35b4847b`): Import flow expansion.** Address fields on clients (migration + mapping). Smart-content detection (samples column values to handle non-standard headers like Contact1 with emails). Preview screen with first-5-row table before any DB write. Phone formatter library (Design Principle #16: store canonical, display human). SOAP-style notes captured into bookings.notes. ImportedDataFootnote component appears on Billing when therapist has imported data. Schedule booking cap raised 2000 to 5000. Beforeunload warning during import. Migration `2026-05-21-clients-address-fields.sql` to apply.
