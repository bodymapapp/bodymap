...
**Most recent activity:** May 24 2026 marathon session (evening into 11pm Central). ~15 commits covering: Phase 13.9-13.11 findOrCreateClient rewrite (phone fallback + stub enrichment + name-match requirement for household-phone case), Phase 13.12 inline ClientPicker in CheckoutModal (sage banner + search + add-new, with onClientLinked patch for slide-over header), package add-vs-charge split (mirrors membership flow, separate Add package + Charge $X buttons), package paidPackageIds in usePackageData hook, removal of duplicate ClientPackageBalance from Sessions and SOAP notes section, yearly view clickable cells with day-detail modal, schedule empty-state copy fix (removed inaccurate "CSV imports don't carry future bookings"), CheckoutModal subtitle precedence fix, prominent Done button + scroll-back-to-section on close, context-aware SuccessView copy. **Cron rebuild (item 27) shipped same session**: all 6 broken crons rebuilt with hardcoded service_role auth + practice-pulse logic switched from sessions to bookings + skipped reasons in response. Practice Pulse force-fire confirmed end-to-end (email landed in inbox). **Customer triage**: Terra's 9 broken bookings linked via white-glove SQL with phone+name match basis (2 held for her morning input: Kare, Maria Cruz). Candice's 3 future-dated broken bookings deliberately untouched per HK; she'll resolve via the new self-serve ClientPicker. Earlier in same day: May 21 marathon (Candice RLS root cause, Jackie's catastrophic-then-recovered import, mobile-first block panel, full-day block render, Schedule 365-day window, etc).
