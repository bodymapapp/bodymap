...
8. **Audit: server-side guards on every 'hide from public' surface (~60 min).** HK May 19 2026 from Candice three-bug report (commit 1f4705df). Pattern: client-side query filters get defeated by cached pages or bookmarks. Today's fix added re-check at booking submit for service.visibility, service.active, and blocked_days overlap. Need same treatment for: private memberships at membership-purchase submit, private packages at package-purchase submit, inactive therapist_locations at booking submit (the location picker filter is client-side too), inactive add-ons at booking submit. Easy template: re-fetch the resource by id at submit, reject if it has flipped to private/inactive/archived/blocked, render rose banner via setSubmitError pattern.
9. **Audit: symmetric-case review of all booking constraint logic (~90 min).** HK May 19 2026 root-cause from buffer bug (one-sided buffer let new slots land directly BEFORE existing bookings). Audit candidates: (a) cancellation policy fires for late cancel but does it also fire for late reschedule, (b) lead-time minimum_advance_hours has a counterpart maximum_advance_days, verify both fire on booking page and BookingModal, (c) lapsed client outreach has a min N days but no max ceiling, may be outreaching stale clients incorrectly, (d) blocked-day overlap currently checks if booking overlaps a partial block but does it also block when an EXISTING booking conflicts with a newly-added block (i.e., warn the therapist), (e) buffer applied to external_calendar_events too. Write tests for each.
