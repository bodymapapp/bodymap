...
19. **Booking-screen display of booked_by + booking_method (Scope B part 3).** QUEUED, HK Jun 1 2026. Storage (migration) and import mapping for these two appointment-provenance fields shipped Jun 1 (commit `eef47583`); the only remaining piece is showing/editing them as pills on the Schedule appointment-detail surface (BookingDetailPage / SidePanel / SessionDetail). Date-sensitive surface, so Rule #35 applies: must be viewed at /founder/mobile-preview before push. Low urgency until appointment history is actually imported (no appointments carry these values yet). Use the same PillRow pattern shipped in ClientProfile/AboutCard. Estimated 45 minutes including mobile QA.
...
## C11 client payment-request email never sends in the fee path (HK Jun 1 2026, CONFIRMED via logs)
Tested on the fixed code (no-show at 22:31 UTC, C11 fix deployed 22:20). Result: payment-link path ran, $1 Square link created (cancellation_charges pending), notify-booking-event returned 200, therapist email/SMS/push all logged. But NO client-audience row in notification_log at all, not even a failure. So clientEmailContentFor's no_show_payment_request branch either returned null or notifyClient silently no-opped. Strong suspicion: the inline client fan-out (fireDownstreamForBookingEvent -> notifyClient) depends on SUPABASE_ANON_KEY which the function's own comment (line ~396) warns is "not auto-provided in this project"; the therapist path uses a different working mechanism, which is why therapist channels land and the client one silently fails. NEXT: confirm whether notifyClient needs that key / why the branch produced no log row; the C11 routing fix (526af0cb) is correct but delivery is the remaining gap. Until fixed, a real no-show client owing a fee with no card gets NOTHING (no payment request).

## no_show double-fires the whole flow (HK Jun 1 2026, CONFIRMED)
On one no-show, logs show TWO create-cancellation-fee-link POSTs and TWO notify-booking-event POSTs ~12s apart, producing two pending cancellation_charges rows ($1 each) and duplicate therapist notifications. Likely the glitchy side panel let the therapist tap twice (no in-flight guard / no idempotency on the modal action). Add a busy/disable guard and/or idempotency so one no-show = one charge + one notification set.
