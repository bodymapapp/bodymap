...
export function detectCsvType(headers, rows = []) {
  ...
  if (effectiveDate && (effectiveTime || hasDuration) && (hasFullName || hasFirstName)) {
    const mapping = {
      ...
      bookedBy: findStrict(h, 'booked by', 'created by', 'scheduled by'),
      bookingMethod: findStrict(h, 'booking method', 'booking type', 'source', 'channel'),
  ...
  if ((hasFirstName || hasLastName || hasFullName) && (effectiveEmail || effectivePhone) && !hasAppointmentSignal) {
    const mapping = {
      ...
      // Scope B (HK Jun 1 2026): extra client fields from competitor exports
      birthday: findLoose(h, 'birthday', 'birthdate', 'date of birth', 'dob', 'birth date'),
      gender: findStrict(h, 'gender', 'sex'),
      referralSource: findLoose(h, 'referred by', 'refered by', 'referral source', 'source', 'how did you hear', 'lead source'),
      customerSince: findLoose(h, 'customer since', 'date added', 'date_added', 'member since', 'client since', 'created'),
      altPhone: pickColumn(findLoose(h, 'landline_number', 'landline', 'day phone', 'night phone', 'alt phone', 'alternate phone', 'home phone', 'work phone', 'secondary phone'), 'phone'),
