...
export default function PostSessionBrief() {
  ...
  const patterns = useMemo(() => {
    ...
    const result = [];
    const pressures = data.history.filter(s => s.pressure).map(s => s.pressure);
    if (pressures.length >= 2) {
      const avg = Math.round(pressures.reduce((a,b)=>a+b,0)/pressures.length);
      result.push(`Consistent pressure preference: Level ${avg}/5 across ${pressures.length} sessions.`);
    }
    const avoidCounts = {};
    data.history.forEach(s => {
      [...(s.front_avoid||[]),...(s.back_avoid||[])].forEach(a => { avoidCounts[an(a)]=(avoidCounts[an(a)]||0)+1; });
    });
    const topAvoids = Object.entries(avoidCounts).filter(([,c])=>c>=2).sort((a,b)=>b[1]-a[1]).slice(0,2).map(([name,c])=>`${name} (${c}/${data.history.length} sessions)`);
    if (topAvoids.length > 0) result.push(`Consistently avoided: ${topAvoids.join(", ")}.`);
    return result;
  }, [data]);
  ...
  return (
    <div style={{fontFamily:"system-ui,sans-serif",background:"white",minHeight:"100vh",color:"#1A1A2E"}}>
      ...
      <div style={{maxWidth:"740px",margin:"0 auto",padding:"24px"}}>
        <div style={{background:"linear-gradient(135deg,#2A5741,#4A8B6B)",borderRadius:"12px",padding:"20px 24px",marginBottom:"16px",display:"flex",justifyContent:"space-between",alignItems:"center",color:"white"}}>
          <div>
            <div style={{fontSize:"11px",opacity:0.8,marginBottom:"3px",textTransform:"uppercase",letterSpacing:"0.5px"}}>Post-Session Brief</div>
            <div style={{fontSize:"22px",fontWeight:"700",fontFamily:"Georgia,serif"}}>Hi {firstName}!</div>
        ...
        {patterns.length > 0 && (
          <div style={{background:"#F0F7F3",border:"1px solid #6B9E8040",borderRadius:"8px",padding:"12px",marginBottom:"14px"}}>
            <div style={{fontSize:"10px",fontWeight:"700",color:"#2A5741",textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:"6px"}}>📊 What We've Learned About You</div>
