  ...
  return (
    <div style={{ minHeight: "100vh", background: C.cream, padding: "24px 16px 48px", fontFamily: "system-ui" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        ...
        <CommsLogGrid rows={filtered} onAfterBackfill={fetchAll} />
...
// if that type has been sent. Scannable at a glance so we can see who's
// been emailed too much and who hasn't been touched. Also hosts the
// Resend backfill trigger to import historical sends.
// ========================================================================
...
function BackfillButton({ onDone }) {
  ...
  const run = async () => {
    if (status === "running") return;
    if (!window.confirm("Import historical email sends from Resend into the comms log?\n\nPulls up to the last 1000 emails and inserts matched ones into notification_log. Safe to run multiple times (dedupes by provider_id). Takes about 15 seconds.")) {
      return;
    }
    setStatus("running");
    setResult(null);
    try {
      ...
      if (error || !data || !data.ok) {
        setStatus("failed");
        setResult({ error: (error && error.message) || (data && data.error) || "unknown" });
        return;
      }
      setStatus("done");
      setResult(data);
      if (onDone) { try { await onDone(); } catch (_e) { /* non-blocking */ } }
  ...
  const label = status === "running" ? "Importing..."
    : status === "done" ? ("Imported " + (result && result.inserted != null ? result.inserted : 0))
    : status === "failed" ? "Failed"
    : "Import from Resend";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 3, alignItems: "flex-end" }}>
      <button
        ...
        title="One-shot import of historical Resend sends into notification_log. Idempotent, admin-only."
      >
        {label}
      </button>
      {status === "done" && result && (
        <div style={{ fontSize: 10, color: C.gray, fontStyle: "italic", maxWidth: 320, textAlign: "right" }}>
          {result.inserted} new, {result.skipped_already_logged} already logged, {result.skipped_unknown_type} unknown type, {result.skipped_no_therapist_match} non-therapist
        </div>
      )}
      {status === "failed" && result && result.error && (
        <div style={{ fontSize: 10, color: C.fall, maxWidth: 280, textAlign: "right" }}>{result.error}</div>
      )}
    </div>
  );
}

function CommsLogGrid({ rows, onAfterBackfill }) {
  ...
  const rowsWithSends = useMemo(() => {
    ...
    return (rows || []).map(function(t) {
      const sendMap = {};
      const history = t.contact_history || [];
      for (let i = 0; i < history.length; i++) {
        const h = history[i];
        const fullKey = autoTypes.indexOf(h.type) !== -1 ? h.type : ("founder_outreach_" + h.type);
        if (!sendMap[fullKey]) sendMap[fullKey] = { count: 0, mostRecent: null, subject: null };
        sendMap[fullKey].count++;
        if (!sendMap[fullKey].mostRecent || new Date(h.sent_at) > new Date(sendMap[fullKey].mostRecent)) {
          sendMap[fullKey].mostRecent = h.sent_at;
          sendMap[fullKey].subject = h.subject;
        }
      }
      let total = 0;
      Object.keys(sendMap).forEach(function(k) { total += sendMap[k].count; });
  ...
  const displayRows = useMemo(() => {
    let r = rowsWithSends;
    if (hideInactive) r = r.filter(function(t) { return t.totalSends > 0; });
    if (sortBy === "total_desc") r = r.slice().sort(function(a, b) { return b.totalSends - a.totalSends; });
    else if (sortBy === "total_asc") r = r.slice().sort(function(a, b) { return a.totalSends - b.totalSends; });
  ...
  const firstManualIndex = OUTREACH_COLUMNS.findIndex(function(c) { return c.group === "manual"; });
  const mint = "#E8F0EA";
  const navyBg = "#E4E8EF";
  ...
  return (
    <div style={{ marginTop: 36 }}>
      <SectionLabel>Comms Log, every email sent to each therapist</SectionLabel>
      ...
      <div style={{ marginBottom: 10, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <div style={{ fontSize: 11, color: C.gray, marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <BackfillButton onDone={onAfterBackfill} />
        </div>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "Georgia, serif", fontSize: 12 }}>
          <thead>
            <tr style={{ background: C.softCream }}>
              ...
              {OUTREACH_COLUMNS.map(function(col, i) {
                return (
          ...
          <tbody>
            {displayRows.length === 0 ? (
              <tr>
                <td colSpan={OUTREACH_COLUMNS.length + 2} style={{ padding: 32, textAlign: "center", color: C.gray, fontSize: 12, fontStyle: "italic" }}>
                  No contacts logged yet. Click Import from Resend above to pull in historical sends, or uncheck "Hide therapists with no sends" to show all rows.
                </td>
              </tr>
            ) : displayRows.map(function(t) {
              return (
                <tr key={t.id} style={{ borderBottom: "1px solid " + C.light }}>
                  <td style={{ padding: "8px 10px", position: "sticky", left: 0, background: "#fff", zIndex: 1, borderRight: "1px solid " + C.light }}>
                    ...
                    {t.last_contact_at && (
                      <div style={{ fontSize: 10, color: C.gray, marginTop: 2, fontStyle: "italic" }}>
                        last: {daysAgoShort(t.last_contact_at)}
                      </div>
                    )}
                  </td>
                  {OUTREACH_COLUMNS.map(function(col, i) {
                    const send = t.sendMap[col.key];
                    ...
                    return (
                      <td key={col.key} style={{
                        ...
                        {hasSend ? (
                          <div title={col.label + ": " + send.count + "x, last " + daysAgoShort(send.mostRecent) + (send.subject ? (", " + send.subject) : "")}
      ...
      <div style={{ marginTop: 10, display: "flex", gap: 16, fontSize: 11, color: C.gray, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div><span style={{ color: C.fall, fontWeight: 700 }}>xN</span> = sent that type N times</div>
